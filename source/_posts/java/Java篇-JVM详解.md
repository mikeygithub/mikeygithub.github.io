...
![image-20200705094149223](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705094149223.png)

这里的四者之间是`包含关系`，不是上层和下层，也不是子系统的继承关系。
...
public class ClassLoaderTest {
    public static void main(String[] args) {
        ...
        System.out.println("试图获取根加载器:"+bootstrapClassLoader);
...
public class ClassLoaderTest {
    ...
    public static void main(String[] args) {
        ...
        //*********启动类加载器************
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/resources.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/rt.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/sunrsasign.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/jsse.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/jce.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/charsets.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/lib/jfr.jar
        //file:/home/mikey/DevTools/jdk1.8.0_221/jre/classes
        //
...
![image-20200705103516138](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705103516138.png)

sun.misc.Launcher 它是一个java虚拟机的入口应用

![image-20200705103636003](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705103636003.png)
...
![image-20200705105151258](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705105151258.png)
...
![image-20200705105810107](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705105810107.png)
...
![image-20200705111640511](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705111640511.png)

当我们通过前面的：类的加载-> 验证 -> 准备 -> 解析 -> 初始化 这几个阶段完成后，就会用到执行引擎对我们的类进行使用，同时执行引擎将会使用到我们运行时数据区

![image-20200705111843003](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705111843003.png)

也就是大厨做饭，我们把大厨后面的东西（切好的菜，刀，调料），比作是运行时数据区。而厨师可以类比于执行引擎，将通过准备的东西进行制作成精美的菜品

![image-20200705112036630](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705112036630.png)
...
![image-20200705112416101](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705112416101.png)
...
![image-20200705112601211](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705112601211.png)
...
![image-20210225150048654](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20210225150048654.png)
...
![image-20210225150239195](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20210225150239195.png)

- 它是一块很小的内存空间，几乎可以忽略不证。也是运行速度最快的存储区域。|
- 在JVM规范中，每个线程都有它自己的程序计数器，是线程私有的，生命周期与线程的生命周期保持一致。
- 任何时间一一个线程都只有一一个方法在执行，也就是所谓的当前方法。程序计数器会存储当前线程正在执行的Java方法的JVM指令地址:或者，如果是在执行native方法，则是未指定值(undefned)。
- 它是程序控制流的指示器，分支、循环、跳转、异常处理、线程恢复等基础功能都需要依赖这个计数器来完成。
- 字节码解释器工作时就是通过改变这个计数器的值来选取下一条需要执行的字节码指令。
- 它是唯一一 个 在Java虚拟机规范中没有规定任何OutOtMemoryError情况的区域。
...
通过PC寄存器，我们就可以知道当前程序执行到哪一步了 ![image-20200705161007423](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705161007423.png)
...
![image-20200705161409533](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705161409533.png)
...
![image-20200705161812542](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705161812542.png)
...
![image-20200705161849557](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705161849557.png)
...
![image-20200705163928652](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705163928652.png)
...
![image-20200705164722033](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705164722033.png)
...
![image-20200705165025382](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705165025382.png)
...
![image-20200705203142545](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705203142545.png)
...
![image-20200705203916023](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705203916023.png)
...
![image-20200705204836977](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705204836977.png)

并行每个线程下的栈都是私有的，因此每个线程都有自己各自的栈，并且每个栈里面都有很多栈帧，栈帧的大小主要由局部变量表 和 操作数栈决定的

![image-20200705205443993](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705205443993.png)
...
![image-20200705212454445](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705212454445.png)

### Slot的重复利用

栈帧中的局部变量表中的槽位是可以重用的，如果一个局部变量过了其作用域，那么在其作用域之后申明的新的局部变就很有可能会复用过期局部变量的槽位，从而达到节省资源的目的。

![image-20200705213106749](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200705213106749.png)
...
![image-20200706090618332](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706090618332.png)

代码举例

![image-20200706090833697](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706090833697.png)
...
![image-20200706092610730](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706092610730.png)
...
![image-20200706093131621](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706093131621.png)

执行完后，让PC + 1，指向下一行代码，下一行代码就是将操作数栈的元素存储到局部变量表1的位置，我们可以看到局部变量表的已经增加了一个元素

![image-20200706093251302](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706093251302.png)
...
![image-20200706093646406](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706093646406.png)

![image-20200706093751711](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706093751711.png)

然后从局部变量表中，依次将数据放在操作数栈中

![image-20200706093859191](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706093859191.png)

![image-20200706093921573](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706093921573.png)

然后将操作数栈中的两个元素执行相加操作，并存储在局部变量表3的位置

![image-20200706094046782](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706094046782.png)

![image-20200706094109629](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706094109629.png)
...
![image-20200706100311886](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706100311886.png)
...
![image-20200706101251847](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706101251847.png)
...
![image-20200706144954070](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706144954070.png)
...
![image-20200706154554604](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706154554604.png)
...
![image-20200706164139252](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706164139252.png)
...
![image-20200706174708418](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706174708418.png)
...
![image-20200706195127740](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706195127740.png)
...
![image-20200706200739392](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706200739392.png)
...
![image-20200706201904057](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706201904057.png)
...
![image-20200706203419496](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706203419496.png)

堆空间内部结构，JDK1.8之前从永久代  替换成 元空间

![image-20200706203835403](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706203835403.png)
...
![image-20200706205756045](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706205756045.png)

```
-XX:+PrintGCDetails
```

![image-20200706205821919](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706205821919.png)

# 70-OOM的说明与举例

### OutOfMemory举例

![image-20200706205947535](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706205947535.png)

![image-20200706210000461](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706210000461.png)
...
![image-20200706211652779](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200706211652779.png)
...
![image-20200707075847954](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707075847954.png)

下面这参数开发中一般不会调：

![image-20200707080154039](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707080154039.png)
...
![image-20200707084208115](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707084208115.png)
...
![image-20200707084714886](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707084714886.png)
...
![image-20200707085232646](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707085232646.png)

我们继续不断的进行对象生成 和 垃圾回收，当Survivor中的对象的年龄达到15的时候，将会触发一次 Promotion晋升的操作，也就是将年轻代中的对象  晋升到 老年代中

![image-20200707085737207](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707085737207.png)
...
![image-20200707091058346](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707091058346.png)
...
![垃圾回收](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/垃圾回收.gif)
...
![image-20200707095606813](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707095606813.png)
...
![image-20200707101511025](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707101511025.png)

其实不分代完全可以，分代的唯一理由就是优化GC性能。如果没有分代，那所有的对象都在一块，就如同把一个学校的人都关在一个教室。GC的时候要找到哪些对象没用，这样就会对堆的所有区域进行扫描。而很多对象都是朝生夕死的，如果分代的话，把新创建的对象放到某一地方，当GC的时候先把这块存储“朝生夕死”对象的区域进行回收，这样就会腾出很大的空间出来。

![image-20200707101543871](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707101543871.png)
...
![image-20200707103547712](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707103547712.png)
...
![image-20200707104253530](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707104253530.png)
...
![image-20200707203038615](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707203038615.png)
...
![image-20200707203441718](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707203441718.png)
...
![image-20200707205634266](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200707205634266.png)
...
![image-20200708093918121](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708093918121.png)

从线程共享与否的角度来看

![image-20200708094507624](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708094507624.png)
...
![image-20200708094747667](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708094747667.png)
...
![image-20200708095853544](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708095853544.png)
...
![image-20200708102919149](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708102919149.png)

而到了JDK8，终于完全废弃了永久代的概念，改用与JRockit、J9一样在本地内存中实现的元空间（Metaspace）来代替

![image-20200708103055914](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708103055914.png)
...
![image-20200708111756800](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708111756800.png)
...
# 92-方法区的内部结构![image-20200708161728320](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708161728320.png)

《深入理解Java虚拟机》书中对方法区（Method Area）存储内容描述如下：它用于存储已被虚拟机加载的类型信息、常量、静态变量、即时编译器编译后的代码缓存等。

![image-20200708161856504](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708161856504.png)
...
![image-20200708171151384](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708171151384.png)
...
![image-20200708172357052](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708172357052.png)
...
![image-20200708204750374](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708204750374.png)

首先现将操作数500放入到操作数栈中

![image-20200708204953552](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708204953552.png)

然后存储到局部变量表中

![image-20200708205029376](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708205029376.png)

然后重复一次，把100放入局部变量表中，最后再将变量表中的500 和 100 取出，进行操作

![image-20200708205221737](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708205221737.png)

将500 和 100 进行一个除法运算，在把结果入栈

![image-20200708205413721](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708205413721.png)

在最后就是输出流，需要调用运行时常量池的常量

![image-20200708205708057](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708205708057.png)

最后调用invokevirtual（虚方法调用），然后返回

![image-20200708205909176](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708205909176.png)

返回时

![image-20200708210540696](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708210540696.png)
...
![image-20200708211541300](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708211541300.png)

JDK7的时候

![image-20200708211609911](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708211609911.png)

JDK8的时候，元空间大小只受物理内存影响

![image-20200708211637952](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708211637952.png)
...
![image-20200708215025527](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708215025527.png)
...
![image-20200708215218078](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708215218078.png)
...
![image-20200708220303243](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200708220303243.png)
...
![image-20200709095356247](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709095356247.png)
...
![image-20200709151033237](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709151033237.png)
...
![image-20200709152801713](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709152801713.png)

# 106-对象访问定位

JVM是如何通过栈帧中的对象引用访问到其内部的对象实例呢？

![image-20200709164149920](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709164149920.png)

### 对象访问的两种方式

#### 句柄访问

![image-20200709164342002](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709164342002.png)
...
![image-20200709164350466](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709164350466.png)
...
![image-20200709170907611](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709170907611.png)
...
![image-20200709230647277](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200709230647277.png)
...
![image-20200710080707873](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710080707873.png)
...
![image-20200710081118053](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710081118053.png)

那么，如果想要让一个Java程序运行起来，执行引擎（Execution Engine）的任务就是将字节码指令解释/编译为对应平台上的本地机器指令才可以。简单来说，JVM中的执行引擎充当了将高级语言翻译为机器语言的译者。

![image-20200710081259276](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710081259276.png)
...
![image-20200710081627217](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710081627217.png)
...
![image-20200710082141643](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710082141643.png)

Java代码编译是由Java源码编译器来完成，流程图如下所示：

![image-20200710082433146](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710082433146.png)

Java字节码的执行是由JVM执行引擎来完成，流程图 如下所示

![image-20200710083036258](https://gitee.com/moxi159753/LearningNotes/raw/master/JVM/1_%E5%86%85%E5%AD%98%E4%B8%8E%E5%9E%83%E5%9C%BE%E5%9B%9E%E6%94%B6%E7%AF%87/12_%E6%89%A7%E8%A1%8C%E5%BC%95%E6%93%8E/images/image-20200710083036258.png)

我们用一个总的图，来说说 解释器和编译器

![image-20200710083656277](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710083656277.png)
...
![image-20200710085323733](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710085323733.png)
...
![image-20200710085553258](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710085553258.png)
...
![image-20200710090203674](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710090203674.png)
...
![image-20200710095417462](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710095417462.png)
...
![image-20200710101829934](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710101829934.png)
...
![image-20200710103103869](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710103103869.png)
...
![image-20200710103340273](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200710103340273.png)
...
![image-20200711093546398](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711093546398.png)

![image-20200711093558709](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711093558709.png)
...
![image-20200711102231129](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711102231129.png)
...
![image-20200711145925091](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711145925091.png)
...
![image-20200711150859709](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711150859709.png)
...
![image-20200711151326909](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711151326909.png)

针对下面这题，在JDK6和8中表现的是一样的

![image-20200711151433277](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200711151433277.png)
...
![image-20200712084539884](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712084539884.png)

## 什么是垃圾

在提到什么是垃圾之前，我们先看下面一张图

![image-20200712085456113](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712085456113.png)
...
![image-20200712090848669](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712090848669.png)
...
![image-20200712092427246](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712092427246.png)
...
![image-20200712102205795](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712102205795.png)
...
![image-20200712103230349](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712103230349.png)
...
![image-20200712104149246](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712104149246.png)

官场上的裙带关系，可达性分析在人类关系网中

![image-20200712104312406](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712104312406.png)
...
![image-20200712104622677](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712104622677.png)
...
![image-20200712110411885](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712110411885.png)
...
![image-20200712112026317](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712112026317.png)
...
![image-20200712112512720](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712112512720.png)
...
![image-20200712113256075](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712113256075.png)
...
![image-20200712150229048](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712150229048.png)

然后我们通过线程，还能够定位到哪里出现OOM

![image-20200712150303710](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712150303710.png)
...
![image-20200712150935078](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712150935078.png)
...
![image-20200712151916991](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712151916991.png)

把可达的对象，直接复制到另外一个区域中复制完成后，A区就没有用了，里面的对象可以直接清除掉，其实里面的新生代里面就用到了复制算法

![image-20200712152029615](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712152029615.png)
...
![image-20200712152847218](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712152847218.png)
...
![image-20200712153236508](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712153236508.png)
...
![image-20200712165318590](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712165318590.png)
...
![image-20200712195158470](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712195158470.png)
...
![image-20200712202522051](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712202522051.png)
...
![image-20200712202822129](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712202822129.png)
...
![image-20200712203607845](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712203607845.png)
...
![image-20200712203815517](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712203815517.png)
...
.![image-20200712205813321](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712205813321.png)
...
![image-20200712211501377](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712211501377.png)
...
![image-20200712211732976](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200712211732976.png)
...
![image-20200713083030867](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713083030867.png)
...
![image-20200713083443486](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713083443486.png)
...
![image-20200713084726176](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713084726176.png)
...
![image-20200713085306400](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713085306400.png)
...
![image-20200713093551365](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713093551365.png)

### 7款经典收集器与垃圾分代之间的关系

![image-20200713093757644](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713093757644.png)
...
![image-20200713094745366](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713094745366.png)
...
![image-20200713100703799](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713100703799.png)
...
![image-20200713102030127](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713102030127.png)
...
![image-20200713110359441](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713110359441.png)
...
![image-20200713205154007](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713205154007.png)
...
![image-20200713212230352](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713212230352.png)
...
![image-20200713215105293](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713215105293.png)

而是这样的一个区域

![image-20200713215133839](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713215133839.png)
...
![image-20200713223244886](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713223244886.png)
...
![image-20200713223509993](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713223509993.png)
...
![image-20200713224113996](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713224113996.png)
...
![image-20200713224716715](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713224716715.png)
...
![image-20200713225100632](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713225100632.png)
...
![image-20200713225810871](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200713225810871.png)
...
![image-20200714075738203](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714075738203.png)

GC发展阶段：Seria l=> Parallel（并行）=> CMS（并发）=> G1 => ZGC

不同厂商、不同版本的虚拟机实现差距比较大。HotSpot虚拟机在JDK7/8后所有收集器及组合如下图

![image-20200714080151020](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714080151020.png)
...
![image-20200714081610474](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714081610474.png)

参数解析

![image-20200714081622526](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714081622526.png)
...
![image-20200714081909309](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714081909309.png)

参数解析

![image-20200714081925767](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714081925767.png)
...
![image-20200714082555688](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714082555688.png)

### FullGC图片、

![image-20200714082714690](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714082714690.png)
...
![image-20200714083332238](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714083332238.png)

然后我们将4M对象存入到Eden区中

![image-20200714083526790](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714083526790.png)
...
![image-20200714084921184](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714084921184.png)

**GC easy**

![image-20200714084726824](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714084726824.png)
...
![image-20200714090608807](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714090608807.png)
...
![image-20200714091201073](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714091201073.png)

停顿时间对比

![image-20200714091401511](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714091401511.png)

虽然ZGC还在试验状态，没有完成所有特性，但此时性能已经相当亮眼，用“令人震惊、革命性”来形容，不为过。 未来将在服务端、大内存、低延迟应用的首选垃圾收集器。

![image-20200714093243028](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714093243028.png)
...
![image-20200714093604012](https://cdn.jsdelivr.net/gh/mikeygithub/jsDeliver@master/resource/img/image-20200714093604012.png)
